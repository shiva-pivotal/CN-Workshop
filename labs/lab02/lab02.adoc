...
. Access the application again http://localhost:8080/cities. Notice that hypermedia for a new `search` endpoint has appeared.
+
[source,bash]
----
~ » curl -i localhost:8080/cities
...
. Access the application at the random route provided by CF or you can do *cf apps* find out the url for corresponding app:
+
[source,bash]
----
$ curl -i https://cloud-native-spring-<random>.cfapps.io/cities or open it in your browser to test it out.
----
