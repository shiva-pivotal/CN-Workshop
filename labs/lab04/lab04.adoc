...
[source,bash]
----
CN-Workshop/labs/my_work/cloud-native-spring $ cf update-service config-server -c '{"git": { "uri": "https://github.com/shiva-pivotal/app-config.git", "label": "master" } }'
----
...
[source,bash]
----
CN-Workshop/labs/my_work/cloud-native-spring $ ./mvnw clean install
----
...
[source,bash]
----
CN-Workshop/labs/my_work/cloud-native-spring $ cf set-env cloud-native-spring TRUST_CERTS https://api.run.haas-81.pez.pivotal.io
CN-Workshop/labs/my_work/cloud-native-spring $ cf set-env cloud-native-spring CF_TARGET https://api.run.haas-81.pez.pivotal.io
...
. If you navigate to the GIT repo we specified for our configuration, https://github.com/shiva-pivotal/app-config.git, you'll see a file named `cloud-native-spring.yml`.  This filename is the same as our `spring.application.name` value for our Spring Boot application.  The configuration is read from this file, in our case the following property:
+
[source,yaml]
----
greeting: Bonjour
...
. Add @RefreshScope annotation to the CloudNativeSpringApplication.java:
...
-----
. Run Maven task *clean* and *install*. Go to the command prompt and do  *cf push*. After the application is pushed, visit the application in the browser. It will show the updated *greeting* value - we will dynamically change and refresh it shortly.

+
image::images/config-app-value-after-restart.png[]

. Now, lets update the Git repo value to 'Hi Refreshed Again!' :
+
image::images/config-app-value-refreshed-again.png[]
...
[source, bash]
-----
$ curl -X POST <APP_URL>/refresh
-----
+
Postman (notice the Status: 200 OK):
+
image::images/config-refresh-with-postman.png[]
...
+
image::images/config-success-with-refresh.png[]

. Congrats! You have successfully refreshed your git repo values dynamically! 
